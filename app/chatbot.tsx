...
const modelRoutes = {
  gpt: "https://privatechat-b4g0apaxfjbzeubr.brazilsouth-01.azurewebsites.net/gpt/chat",
  deepseek: "https://privatechat-b4g0apaxfjbzeubr.brazilsouth-01.azurewebsites.net/deepseek/chat",
};
...
export default function Chatbot() {
  ...
  useEffect(() => {
    axios.get("https://privatechat-b4g0apaxfjbzeubr.brazilsouth-01.azurewebsites.net/sessions", {
  ...
  const renameChat = async (sessionId: string, newName: string) => {
    ...
    try {
      await axios.put(`https://privatechat-b4g0apaxfjbzeubr.brazilsouth-01.azurewebsites.net/sessions/${sessionId}/rename`, { name: newName }, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
      });
    } catch (error) {
  ...
  const startNewChat = async () => {
    ...
    await axios.post("https://privatechat-b4g0apaxfjbzeubr.brazilsouth-01.azurewebsites.net/sessions", newSession, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
    });
};
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const botResponse = response.data.response || "Erro ao processar resposta.";
      let botMessage = { user: "", bot: "" };
      setMessages((prev) => [...prev, botMessage]);
  
      let index = 0;
      const typeMessage = () => {
        if (index < botResponse.length) {
          botMessage = { ...botMessage, bot: botResponse.slice(0, index + 1) };
          setMessages((prev) => [...prev.slice(0, -1), botMessage]); // Atualiza a última mensagem
          index++;
          setTimeout(typeMessage, 30); // Ajuste a velocidade da digitação
        } else {
          setLoading(false);
        }
      };
      ...
      // Atualiza o histórico no backend
      const updatedMessages = [...messages, userMessage, { user: "", bot: botResponse }];
      await axios.put(`https://privatechat-b4g0apaxfjbzeubr.brazilsouth-01.azurewebsites.net/${currentSession}`, {
